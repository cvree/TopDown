...
import { todayKey, trainingPriority, weakestAxis, type HistoryEntry, type Profile } from './profile';
 ...
 */
/** The run history, cut into sittings. Oldest first. */
const groupSessions = (p: Profile): HistoryEntry[][] => {
  const h = p.history;
  if (!h.length) return [];
...
const summarise = (p: Profile, pick: HistoryEntry[], live: boolean): SessionSummary => {
  const h = p.history;
  const first = pick[0];
...
export const lastSession = (p: Profile, includeLive = false): SessionSummary | null => {
  const groups = groupSessions(p);
  if (!groups.length) return null;
  const live = Date.now() - p.history[p.history.length - 1].t < SESSION_GAP_MS;
  const pick = live && !includeLive ? groups[groups.length - 2] : groups[groups.length - 1];
  if (!pick || !pick.length) return null;
  return summarise(p, pick, live && includeLive);
};

/** Every sitting, newest first. The practice history reads off this. */
export const sessionHistory = (p: Profile, limit = 12): SessionSummary[] => {
  const groups = groupSessions(p);
  ...
  return groups
    .slice(-limit)
    .reverse()
    .map((g, i) => summarise(p, g, live && i === 0));
};
