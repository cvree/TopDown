...
export abstract class WasdDrill extends Drill {
  ...
  outcome(): DrillOutcome {
    ...
    /**
     * Did the module get any of what it asked for?
     *
     * Every term above is a ratio, and a ratio over one accidental answer is
     * not a measurement — a player holding a single key and clicking once by
     * chance reads as 100% accurate, 100% on the move, and perfectly economical
     * with a cursor that never went anywhere. Six of the nine modules scored
     * over half for exactly that, and one scored 99.6%.
     *
     * So the whole number is gated on volume: the share of the things this
     * module actually posed that came back answered. A learner getting a
     * quarter of them barely notices this; somebody getting one in forty
     * cannot reach a passing score however flattering their ratios are.
     */
    const answeredShare = this.tasks > 0 ? clamp(this.solved / this.tasks, 0, 1) : 1;
    const engagement = this.tasks > 0 ? band(answeredShare, 0.03, 0.3) : 1;

    /**
     * And did the feet make any decisions?
     *
     * Almost every number this engine keeps is a share of time spent moving,
     * and all of them are free to a player who tapes one key down: uptime is
     * 1.0, every mark is taken "on the move", and a cursor chasing marks while
     * the feet run in a straight line even reads as independent. Six modules
     * scored over half that way, one of them 87%, and no term inside a module
     * could see it — because from inside a module that run looks like somebody
     * who never stopped moving.
     *
     * A heading change is counted on a turn *or* on a fresh press, so the
     * release-and-repress rhythm the cadence modules teach satisfies this as
     * readily as the constant turning the kiting ones do. One decision every
     * two and a half seconds is full marks. Zero is a taped key.
     */
    const steering = band(this.changeRate(), 0.05, 0.4);

    const performance =
      clamp(q * w + disc * (1 - w), 0, 1) * (0.2 + 0.8 * engagement) * (0.25 + 0.75 * steering);
    ...
    if (this.changeRate() < 0.12 && this.s.elapsed > 8)
      hurt.push('Your feet barely changed direction. Holding one key is not movement — it is a direction you picked once.');
    if (m.windupBreaks > 4)
