...
import { todayKey, trainingPriority, type Profile } from '../progression/profile';
import { RankEmblem } from './components/RankEmblem';
import './daily.css';

/** The last seven calendar days, newest last, marked where a programme ran. */
const lastSevenDays = (marks: { date: string }[]): { key: string; label: string; done: boolean }[] => {
  const set = new Set(marks.map((m) => m.date));
  const out: { key: string; label: string; done: boolean }[] = [];
  for (let i = 6; i >= 0; i--) {
    const d = new Date();
    d.setDate(d.getDate() - i);
    const key = todayKey(d);
    out.push({ key, label: 'SMTWTFS'[d.getDay()], done: set.has(key) });
  }
  return out;
};
...
export function Daily({ profile, onStart, onBack }: Props) {
  ...
  const week = lastSevenDays(profile.dailyMarks);
  ...
  return (
    <div className="scroll">
      <div className="wrap daily fade-up">
        ...
        {/* The right column is the reason to come back tomorrow: the streak,
            where the rating stands, and the one axis to aim at next. */}
        <aside className="daily-side">
          <div className="panel pad ds-streak">
            <div className="panel-title">Streak</div>
            <div className="ds-streak-n display">{profile.daily.streak}</div>
            <div className="faint ds-streak-l">consecutive day{profile.daily.streak === 1 ? '' : 's'}</div>
          ...
          <div className="panel pad ds-rank">
            <div className="panel-title">Standing</div>
            <div className="ds-rank-row">
              <RankEmblem tier={rank.tier} size={54} />
              <div>
                <b className="display">{profile.placed ? rank.label : 'UNRANKED'}</b>
                <span className="mono faint">{profile.placed ? `${Math.round(profile.overall)} rating` : 'not calibrated'}</span>
              </div>
            </div>
